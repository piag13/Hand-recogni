...
for j in range(number_of_classes):
    ...
    while True:
        ...
        if hands:
            ...
            Img_black = np.ones((IMG_SIZE, IMG_SIZE, 3), np.uint8) # White background

            # Crop with margin
            x1, y1 = max(x - 20, 0), max(y - 20, 0)
            x2, y2 = x + w + 20, y + h + 20
            crop_image = image[y1:y2, x1:x2]
            imgOut = segmentor.removeBG(image, success[IMG_SIZE], threshold=0.8)
